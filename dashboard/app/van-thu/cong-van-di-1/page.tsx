...
export default function CongVanDi1Page() {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        try {
            const data = await fetchCongVan("Công văn đi 1");
            setRows(data);
    ...
    const COLS = ["STT", "Ngày/Tháng", "Số văn bản", "Ngày văn bản", "Tóm nội dung chính", "Đơn vị gửi đi", "Người nhận", "Bản Scan", "Bản gốc"];

    return (
        <div className="flex min-h-screen">
            <Sidebar />
            <div className="ml-60 flex-1 flex flex-col">
                <Header title="Công Văn Đi 1" subtitle="Danh sách văn bản đi (Công văn đi 1) – Trung Nam E&C" />
                <main className="p-8 space-y-6">
                    <div className="glass rounded-2xl overflow-hidden shadow-xl border border-white/20">
                        <div className="p-5 border-b border-white/40 flex items-center justify-between bg-violet-50/20">
                            <div className="flex items-center gap-2 text-violet-700 font-semibold text-sm">
                                <Send size={16} />
                                <span>{filtered.length} công văn đi</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="relative">
                                    ...
                                    <input value={search} onChange={e => setSearch(e.target.value)}
                                        ...
                                        className="pl-8 pr-4 py-2 text-sm bg-white/80 border border-slate-200 rounded-xl w-64 focus:outline-none focus:ring-2 focus:ring-violet-400/20" />
                                </div>
                                <button onClick={load}
                                    className="flex items-center gap-2 text-sm font-semibold text-violet-700 glass px-4 py-2 rounded-xl transition-all">
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={COLS.length} className="py-20 text-center text-slate-400">
                                            {rows.length === 0 ? "Chưa có dữ liệu công văn đi 1." : "Không tìm thấy kết quả phù hợp."}
                                        </td></tr>
                                    ) : filtered.map((r, i) => (
                                        <tr key={i} className={`border-b border-slate-100 transition-colors ${i % 2 === 0 ? "bg-white/40" : "bg-violet-50/10"} hover:bg-violet-100/20`}>
                                            ...
                                            <td className="px-4 py-3.5 font-semibold text-violet-700 whitespace-nowrap">{r["Số văn bản"]}</td>
